...
class GenericArgument(object):
    def __init__(self, name: str = "unknown", typing = str, inclusion: list = [], exclusion: list = [], necessary: bool = False, args_after: int = 0, value = None, is_help = False):
        self.name = name
        try:
            self.value = (list(map(typing, value)) if isinstance(value, Iterable) and not isinstance(value, (str, bytes, bytearray)) else typing(value)) if value is not None else value
        except ValueError:
...
class ArgumentParserSettings(object):
    ...
    def define_option(self, key, typing = str, name=None, inclusion = [], exclusion = [], necessary = False, args_after: int = 0, is_help = False) -> None:
        self.options[key] = GenericArgument(name if name is not None else key, typing, inclusion, exclusion, necessary, args_after, is_help=is_help)
    ...
    def parse(self):
        ...
        while (i < self._argc):
            ...
            if ('=' in value and value.split('=')[0] in self.settings.parameters):
                ...
                if (self.settings.options[value].is_help):
                    is_help = True
